...
void Compiler::RunCompile(	const std::vector<std::string>&	filesToCompile_,
							const CompilerOptions&						compilerOptions_,
							std::vector<std::string>			linkLibraryList_,
							const std::string&				moduleName_ )
{
	if( m_pImplData->m_bFindVS )
	{
		...
		else
		{
			...
			LOG("No Supported Compiler for RCC++ found.\n");
		}
	}

    if( m_pImplData->m_VSPath.empty() )
    {
        LOG("No Supported Compiler for RCC++ found, cannot compile changes.\n");
...
// Add improved debugging options if available: http://randomascii.wordpress.com/2013/09/11/debugging-optimized-codenew-in-visual-studio-2012/
	...
	if( compilerOptions_.libraryDirList.size() ||  bHaveLinkOptions )
	{
		...
		{
			linkOptions += " /LIBPATH:\"" + compilerOptions_.libraryDirList[i] + "\"";
		}
	...
	if ( !App->fileSystem->Exists(compilerOptions_.intermediatePath.c_str()))
	{
		bool success = App->fileSystem->CreateDir(compilerOptions_.intermediatePath.c_str());

		if( success ) { LOG("Created intermediate folder \"%s\"\n", compilerOptions_.intermediatePath.c_str()); }
		else LOG("Error creating intermediate folder \"%s\"\n", compilerOptions_.intermediatePath.c_str());
	}
	...
	{
		strIncludeFiles += " /I \"" + compilerOptions_.includeDirList[i] + "\"";
	}
	...
	{
		std::string strPath = filesToCompile_[i];
		App->fileSystem->ToLowerInPlace(strPath);
	...
	{
		strLinkLibraries += " \"" + linkLibraryList_[i] + "\" ";
	}
...
char* pCharTypeFlags = "";
	...
	std::string cmdToSend = compilerLocation + flags + pCharTypeFlags
		+ " /MP /Fo\"" + compilerOptions_.intermediatePath + "\\\\\" "
		+ "/D WIN32 /EHa /Fe" + moduleName_;
	cmdToSend += " " + strIncludeFiles + " " + strFilesToCompile + strLinkLibraries + linkOptions
		+ "\necho ";
	LOG( "%s", cmdToSend.c_str() ); // use %s to prevent any tokens in compile string being interpreted as formating
...
void GetPathsOfVisualStudioInstalls( std::vector<VSVersionInfo>* pVersions)
{
	...
	case 1927: // VS 2019
		...
		break;
	default:
		LOG("WARNING: VS Compiler with _MSC_VER %d potentially not supported. Defaulting to version %s.\n", MSCVERSION, VS_DISCOVERY_INFO[startVersion].valueName);
	}
#endif
...
void ReadAndHandleOutputThread( LPVOID arg )
{
	...
	while( bReadActive )
	{
										...
										&nBytesRead,NULL) || !nBytesRead)
		{
			...
			{
				LOG( "[RuntimeCompiler] Redirect of compile output failed on read\n" );
			}
		}
		else
		{
			...
			if( found != std::string::npos )
			{
				...
				if( !pCmdProc->m_bStoreCmdOutput) LOG("[RuntimeCompiler] Complete\n");
...
void CmdProcess::InitialiseProcess()
{
	...
	{
		LOG("[RuntimeCompiler] Failed to create output redirection pipe\n");
		goto ERROR_EXIT;
	}
	...
	{
		LOG("[RuntimeCompiler] Failed to duplicate error output redirection pipe\n");
		goto ERROR_EXIT;
	}
	...
	if (si.hStdOutput)
	{
		...
		{
			LOG("[RuntimeCompiler] Failed to duplicate output read pipe\n");
			goto ERROR_EXIT;
		}
	...
	{
		LOG("[RuntimeCompiler] Failed to create input pipes\n");
		goto ERROR_EXIT;
	}
	...
	if (si.hStdOutput)
	{
		if (!DuplicateHandle(GetCurrentProcess(), hInputWriteTmp,
			...
			DUPLICATE_SAME_ACCESS))
		{
			LOG("[RuntimeCompiler] Failed to duplicate input write pipe\n");
			goto ERROR_EXIT;
		}
	}
	/*
